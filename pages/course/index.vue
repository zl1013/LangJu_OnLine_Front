            ...
            <dd class="c-s-dl-li">
              <ul class="clearfix">
                <li v-for="(SubjectTwo,index) in subSubjectList" :key="index" :class="{active:twoIndex==index}">
                  <a :title="SubjectTwo.title" href="#" @click="serachTwo(SubjectTwo.id,index)">{{SubjectTwo.title}}</a>
                </li>
                ...
                <div class="cc-l-wrap">
                  ...
                  <section class="mt10 hLh20 of">
                    <span class="fr jgTag bg-green" v-if="Number(item.price) === 0">
                      ...
                      <!-- <i class="c-fff fsize12 f-fA" v-if="item.price > 0">{{item.price}}</i> -->
                    </span>
...
export default {
  ...
  methods:{
    ...
    searchOne(subjectParentId,index){
     ...
     initCourseFirst(){
       courseApi.getCourseList(1,4,this.searchObj)
       .then(res => {
     ...
     //分页切换
     gotoPage(page){
       courseApi.getCourseList(page,4,this.searchObj)
       .then(res => {
         this.data = res.data.data
       })
    }
